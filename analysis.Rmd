---
title: "Executive functions predict psychopathic traits in incarcerated violent offender"
author: "Carl Delfin"
date: "4 July 2017"
output:
...
Paths (e.g. to data) and keys (e.g. to Google Sheets) are stored in an external script for security reasons. Every variable written in ALL CAPS is a pre-defined path/key.
...
```{r}
requiredPackages <- c("foreign", "psych", "plyr", "dplyr", "reshape", "ggplot2", "ggcorrplot", "broom", "relaimpo", "lmtest", "caret", "sandwich", "car", "cowplot", "googlesheets")
```
...
The data is read from an SPSS file (not my own choice...), and the seed is set for reproducibility (I always use the year the analysis was conducted, in this case 2017, to avoid "seed hacking"). The `seed` function uses [Mersenne Twister](https://en.wikipedia.org/wiki/Mersenne_Twister) by default.
...
# retrieve Google Drive sheet, store results
...
Before going further let's do a simple diagnostic check on the linear models. Looking at the residual versus fitted plots, we see some [heteroscedasticity](https://en.wikipedia.org/wiki/Heteroscedasticity). Thus, heteroscedasticity-consistent standard errors will be used throughout. Looking at [Q-Q plots](https://en.wikipedia.org/wiki/Q%E2%80%93Q_plot), some deviations from normality are evident, although not enough to warrant further actions.
...
1. Loop through **formulaList** using `lm` to estimate regression models, and store the results in **modelList** 2. Loop through the newly created **modelList** using:
...
Table 2 contains detailed model summaries, such as betas, *p*-values and CIs. See *table2.R* for details. Results are stored in the cloud using `googlesheets`.
...
Relative importance estimates are plotted and saved in pdf. See *relaimpo.R* for details. Note that model RI sum to unadjusted model *R*<sup>2</sup>.
...
```{r}
time <- proc.time() - ptm
paste("analysis took", round(time[3]/60, 0), "minute(s)")
```