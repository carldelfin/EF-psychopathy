...
Flextable is used to create Word ready tables. It took forever to find a decent way to export everything to Word, and I don't particularly like Word, but since the publishing process is what it is... There's still some tinkering that needs to be done manually in Word, such as removing NAs, page orientation, and other cosmetics. The main point is, you don't have to type in a single number by yourself.
```{r}
# create empty list
flextable <- NULL
