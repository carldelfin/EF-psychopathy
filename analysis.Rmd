...
step1ri <- lapply(step1models, calc.relimp, rela = TRUE)                   # relative importance
```

### Build step 2 models
```{r}
step2models <- lapply(step2formulas,                                  # same as above
...
tidys1[[1]]$p.adj <- padj[1:6]     # six Interpersonal facet coefficient q-values
tidys1[[2]]$p.adj <- padj[18:23]   # six Affective facet coefficient q-values
tidys1[[3]]$p.adj <- padj[35:40]   # six Lifestyle facet coefficient q-values
tidys1[[4]]$p.adj <- padj[52:57]   # six Antisocial facet coefficient q-values
tidys1[[5]]$p.adj <- padj[69:74]   # six Total score coefficient q-values

# tidys2, i.e. step 2 models
tidys2[[1]]$p.adj.1 <- padj[8:15]  # same procedure as above,
tidys2[[2]]$p.adj.1 <- padj[25:32] # but now with 8 q-values,
...
for (i in 1:length(tab)) {
  ...
  tab[[i]] <- tab[[i]][c("estimate", "cilow","cihigh", "statistic",
                         "p.value", "p.adj", "estimate.1", "cilow.1", 
                         "cihigh.1", "statistic.1", "p.value.1", "p.adj.1")]
  # rename
  names(tab[[i]]) <-c("B", "95% CI LL", "95% CI UL", "t", "p", "padj", 
                      "B", "95% CI LL", "95% CI UL", "t", "p", "padj")
}
```
...
for (i in 1:length(tab)) {
  ...
  row.names(tab[[i]]) <- c("SOCMITT5", "SOCPS", "IEDstages",
                           ...
                           "Delta F", "Delta Fp", "Delta Fpadj", "Delta CV R2")
}
```
...
```{r}
# model 1
tab[[1]][11,1] <- padj[7]        # step 1 model 1 F q-value
tab[[1]][11,7] <- padj[16]       # step 2 model 1 F q-value
tab[[1]][17,7] <- padj[17]       # model 1 comparison q-value

# model 2
tab[[2]][11,1] <- padj[24]       # step 1 model 2 F q-value
tab[[2]][11,7] <- padj[33]       # step 2 model 2 F q-value
tab[[2]][17,7] <- padj[34]       # model 2 comparison q-value

# model 3
tab[[3]][11,1] <- padj[41]       # step 1 model 3 F q-value
tab[[3]][11,7] <- padj[50]       # step 2 model 3 F q-value
tab[[3]][17,7] <- padj[51]       # model 3 comparison q-value

# model 4
tab[[4]][11,1] <- padj[58]       # step 1 model 4 F q-value
tab[[4]][11,7] <- padj[67]       # step 2 model 4 F q-value
tab[[4]][17,7] <- padj[68]       # model 4 comparison q-value

# model 5
tab[[5]][11,1] <- padj[75]       # step 1 model 5 F q-value
tab[[5]][11,7] <- padj[84]       # step 2 model 5 F q-value
tab[[5]][17,7] <- padj[85]       # model 5 comparison q-value
```
...
**Note** that in the final publication, due to not wanting to cross-report findings in text and tables, not all of the below may actually be included. Better safe than sorry, however, so best to include everything in the tables created here.
```{r}
# create empty list
flextable <- NULL
...
for (i in 1:length(tab)) {
  ...
  tab[[i]]$rownames <- c("SOCMITT5", "SOCPS", "IEDstages", "IEDstageser", "SWMstrategy",
                         "SWMtotaler", "ADHD", "SUD", "F", "Fp", "Fpadj", "CV R2", "CI CV R2 LL",
                         "CI CV R2 UL", "Delta F", "Delta Fp", "Delta Fpadj", "Delta CV R2")
  ...
  flextable[[i]] = addHeaderRow(flextable[[i]], value = c("", "B", "95% LL", "95% UL", "t", "p", "padj",
                                                              "B", "95% LL", "95% UL", "t", "p", "padj"),
  text.properties = textBold())
...
All regression model results are written to five separate Word documents. Could probaly loop this, but oh well.
```{r}
doc <-  docx()
doc <- addFlexTable(doc, flextable[[1]])
writeDoc(doc, file = "results/Table2_Facet1.docx")

doc <-  docx()
doc <- addFlexTable(doc, flextable[[2]])
writeDoc(doc, file = "results/Table3_Facet2.docx")

doc <-  docx()
doc <- addFlexTable(doc, flextable[[3]])
writeDoc(doc, file = "results/Table4_Facet3.docx")

doc <-  docx()
doc <- addFlexTable(doc, flextable[[4]])
writeDoc(doc, file = "results/Table5_Facet4.docx")

doc <-  docx()
doc <- addFlexTable(doc, flextable[[5]])
writeDoc(doc, file = "results/Table6_TotalScore.docx")
```
