...
```{r}
# clear timer and timer log, just in case
tic.clear()
tic.clearlog()
...
```{r}
# select number of MCMC iterations and number of burn-in iterations
niter <- 100
nburnin <- 10

# start timer and grap some popcorn
...
Figure 1 is a three-panel overview Pearson's *r*, *p*-values and Bayes factors arranged in a matrix. See *figure1.R* for details.

```{r}
source("scripts/figure1.R")
ggsave(filename = "hidden/figures/figure1.tiff",
       plot = bayesplot,
       height = 11.69,
       width = 7.5,
       dpi = 300,
