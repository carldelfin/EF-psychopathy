...
plot_bayes <- ggplot(result_full, aes(x = X, y = Y, fill = bayes)) +
  ...
  scale_fill_distiller(type = "div",
                       ...
                       labels = c("0", "1/3", "1", "3+")) +
  geom_text(aes(x = X, y = Y, label = formatC(bayes, format = "E", digits = 2)), size =  2.5) +
  coord_flip() +
  scale_x_discrete(expand = c(0, 0),
                   name = "", 
                   labels = labels) +
...
bayesplot <- plot_grid(plot_cor, plot_p, plot_bayes,
                       ncol = 1, nrow = 3, labels = "AUTO")

rm(cor, result_full, plot_cor, plot_p, plot_bayes, ctitle, ptitle, labels)